...
# Final Project Added
def metropolis_hastings(n_steps, initial_x, step_size, unnormalized_density_func, burn_in=0.1,
                        return_rate=False, verbose=True):
    """
    Runs the Metropolis-Hastings sampler for a given unnormalized density function.
    ...
    current_x = initial_x
    current_p = unnormalized_density_func(current_x) # Use the passed function
    ...
    for i in range(1, total_steps):
        ...
        # 2. Calculate target density at new state
        proposed_p = unnormalized_density_func(proposed_x) # Use the passed function
    ...
    if verbose:
        print(f"[M-H] Step: {step_size:.4f}, Rate: {acceptance_rate:.2%}")
...
def tune_step_size(initial_x, unnormalized_density_func, target_rate=0.4, n_tune_steps=1000, 
                   tolerance=0.05, max_iter=20):
    """
    Tunes the M-H step_size to target an acceptance rate of ~40-50%.
    """
    print(f"[Tuner] Tuning step size (target rate = {target_rate:.0%})...")
    ...
    for i in range(max_iter):
        # Run a short, silent M-H test and get the rate
        _, rate = metropolis_hastings(n_tune_steps, initial_x, step_size, 
                                      unnormalized_density_func, # Pass density func
                                      burn_in=0.2, return_rate=True, verbose=False)
...
def metropolis_step_1d(current_x, step_size, unnormalized_density_func, beta):
    """
    Performs one step of 1D Metropolis-Hastings for a single component.
    Used by the Gibbs sampler for the separable multi-dimensional problem.
    """
    # 1D target density is p(x) = unnormalized_density_func(x, beta)
    # 1. Calculate current probability (using the target density function)
    current_p = unnormalized_density_func(current_x, beta)
    
    # 2. Propose a new state
    proposed_x = current_x + np.random.normal(0, step_size)
    proposed_p = unnormalized_density_func(proposed_x, beta)
    
    # 3. Calculate acceptance probability (alpha)
    if current_p == 0: 
        alpha = 1.0
    else:
        # Symmetric proposal: alpha = min(1, p(x') / p(x))
        alpha = min(1.0, proposed_p / current_p)
